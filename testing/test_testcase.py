...
class WarmupInterval(unittest.TestCase, utilities.partialChecks):
    ...
    def test_environment_variable(self):
        '''Test that BOPTEST_WARMUP_INTERVAL sets the grid and that an
        explicit argument wins over it.

        '''

        os.environ['BOPTEST_WARMUP_INTERVAL'] = '900'
        self.assertEqual(self._make().warmup_interval, 900.)
        self.assertEqual(self._make(warmup_interval=60).warmup_interval, 60.)
    ...
    def tearDown(self):
        os.environ.pop('BOPTEST_WARMUP_INTERVAL', None)
