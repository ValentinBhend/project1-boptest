...
- If your controller re-initializes the test often, for example when training a reinforcement learning agent that starts a new episode from a random day, you can make the warmup simulation cheaper by coarsening the grid it is simulated on with the environment variable ``BOPTEST_WARMUP_INTERVAL`` in the ``.env`` file.  The default of 30 seconds reproduces the behaviour of earlier BOPTEST versions.  Control steps always use 30 seconds whatever this is set to, so the grid the KPIs integrate over does not change, and the warmup samples are excluded from that integration in any case.  What does change is the state reached at the start time, and with it the reported KPIs, by a small amount: measured on ``bestest_hydronic_heat_pump`` with a one day warmup, coarsening the grid to one step over the whole warmup moves the zone air temperature by at most 3e-05 K and the KPIs by at most 1e-05 relative, while making ``initialize`` 2.5x cheaper.  Results requested over the warmup period are returned on the coarser grid.
