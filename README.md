...
- If your controller makes very many short control steps, for example when training a reinforcement learning agent, you can request a low-overhead simulation path by sending ``fast=true`` when selecting the test case.  It is off by default.  This path steps the emulator FMU on the same communication grid as the default one, so measurements and KPIs are unaffected, but it does not write and read back a simulation result file for every control step and it logs less.  The computational time ratio KPI ``time_rat`` is a wall-clock measurement and therefore does change.
...
| Select an IBPSA BOPTEST test case and begin a new test. Returns a `testid` which is required by all APIs that interact with the test or provide test information.                                                            | POST ``testcases/{testcase_name}/select`` with optional argument ``fast=<true or false>``, which requests the low-overhead simulation path described below (default is false). See the [Test Case](https://ibpsa.github.io/project1-boptest/testcases/index.html) page for options and documentation.          |
...
| Receive test KPIs.  All core KPIs are returned unless a subset is requested, which is useful on every control step because the peak demand KPIs cost more the longer the test has run.                                                    |  GET ``kpi/{testid}`` with optional argument ``names=<comma separated list of KPI names>``                                             |
