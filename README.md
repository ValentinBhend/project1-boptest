...
- If your controller makes very many short control steps, for example when training a reinforcement learning agent, you can request a low-overhead simulation path by sending ``fast=true`` when selecting the test case, or turn it on for every test on the deployment by setting the environment variable ``BOPTEST_FAST`` in the ``.env`` file.  It is off by default.  This path steps the emulator FMU on the same communication grid as the default one, so measurements and KPIs are unaffected, but it does not write and read back a simulation result file for every control step and it logs less.  The computational time ratio KPI ``time_rat`` is a wall-clock measurement and therefore does change.
