...
- Add an optional ``names`` argument to the ``GET kpi/{testid}`` API, and a corresponding ``names`` argument to ``KPI_Calculator.get_core_kpis`` and ``TestCase.get_kpis``, to calculate only a subset of the core KPIs.  Omitting it calculates all of them, as before.  This is useful for a controller that queries KPIs on every control step, since the peak demand KPIs are maxima over the whole test period and therefore cost more the longer the test has run.  This is for [#XXX](https://github.com/ibpsa/project1-boptest/issues/XXX).
...
- Add the option to simulate the warmup period on a coarser communication grid than the 30 s one used for control steps, through the new optional ``warmup_interval`` argument to ``PUT initialize/{testid}`` and to ``TestCase.initialize``.  Omitting it keeps the 30 s grid, which reproduces the previous behaviour exactly and therefore does not change benchmark results.  Setting it larger makes ``initialize`` up to 2.5x cheaper, which matters for a controller that re-initializes often, at the cost of moving reported KPIs slightly.  This is for [#XXX](https://github.com/ibpsa/project1-boptest/issues/XXX).  Details if it is enabled:
  - Control steps always use 30 s, so the grid the KPIs integrate over is unchanged, and warmup samples are excluded from that integration in any case.  What changes is the state reached at the start time, because the FMU solver is restarted at every communication point and so takes a different internal path to get there.
  - Measured on ``bestest_hydronic_heat_pump`` with a one day warmup and a 96 step test period, at start days 16 and 300, against the 30 s reference: coarsening to 60 s, 300 s, 900 s or a single step over the whole warmup moves the zone air temperature by at most 3e-05 K and the KPIs by at most 1e-05 relative, always on ``tdis_tot``.  Reset cost falls from 451 ms to 300 ms (60 s) and to 182 ms (single step).
  - The deviation is not monotonic in how coarse the grid is, which indicates solver-tolerance noise rather than discretisation error: the FMU's own error control sets the accuracy, so every variant lands within tolerance of the same solution.
  - Results requested over the warmup period are returned on the coarser grid.
