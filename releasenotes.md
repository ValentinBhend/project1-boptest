...
- Add an optional ``warmup_interval`` argument to ``PUT initialize/{testid}`` and ``TestCase.initialize``, which sets the communication grid for the warmup simulation.  Omitting it keeps the 30 s grid used for control steps.  A coarser grid makes ``initialize`` cheaper and shifts reported KPIs by less than 1e-05 relative.  This is for [#860](https://github.com/ibpsa/project1-boptest/issues/860).
- Replace three pandas calls in ``data/data_manager.py`` and ``kpis/kpi_calculator.py`` that are removed in pandas 3: ``DataFrame.applymap`` becomes ``DataFrame.astype``, ``pd.TimedeltaIndex(..., unit='s')`` becomes ``pd.to_timedelta``, and the ``'15T'`` resample alias becomes ``'15min'``.  The replacements work in pandas 0.24 and later and return the same values.  This is for [#XXX](https://github.com/ibpsa/project1-boptest/issues/XXX).
