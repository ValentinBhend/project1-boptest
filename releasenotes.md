...
- Add an optional low-overhead simulation path, off by default, requested with the new ``fast`` argument to ``POST testcases/{testcase_name}/select`` or to ``TestCase``.  It steps the emulator FMU with ``do_step`` instead of calling ``pyfmi.fmu.simulate`` per control step, on the same communication grid, so only ``time_rat`` changes.  This is for [#XXX](https://github.com/ibpsa/project1-boptest/issues/XXX).
