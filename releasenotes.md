...
- Add the option to simulate the warmup period on a coarser communication grid than the 30 s one used for control steps, through the new optional ``warmup_interval`` argument to ``PUT initialize/{testid}`` and to ``TestCase.initialize``.  The default of 30 s reproduces the previous behaviour exactly and therefore does not change benchmark results.  Setting it larger makes ``initialize`` up to 2.5x cheaper, which matters for a controller that re-initializes often, at the cost of moving reported KPIs slightly.  This is for [#XXX](https://github.com/ibpsa/project1-boptest/issues/XXX).  Details if it is enabled:
