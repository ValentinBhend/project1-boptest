...
class TestCase(object):
    ...
    def get_kpis(self, names=None):
        ...
        names: list of str, optional
            ...
            left out of the payload, which is useful for a controller that
            needs only some of them on every control step: the peak-demand
            KPIs are maxima over the whole test period and so cost more the
            longer the test has run, while the integrated KPIs do not.
            Requesting a subset does not affect the value any KPI reports
            later, because each keeps its own integration index.
