...
class TestCase(object):
    ...
    def __init__(self,
        ...
        forecast_uncertainty_params_path : str, optional
            ...
            Default is assuming a particular directory structure.
        fast : bool, optional
            Set to True to enable the low-overhead simulation path, which
            steps the FMU directly instead of calling ``pyfmi.fmu.simulate``
            once per control step.  The communication grid, and therefore
            every measurement and KPI, is unchanged; what is removed is the
            per-step construction of a simulation algorithm and result
            handler, the result file write and read-back, and the verbose FMU
            and framework logging.
            Default is None, which reads the ``BOPTEST_FAST`` environment
            variable and otherwise falls back to False.  Opt-in in every case.

        '''

        # Resolve the fast path first, since it changes how the FMU and the
        # logger are configured below. An explicit argument beats the
        # environment variable, which in turn beats the default of off.
        if fast is None:
        ...
        # The FMU debug log is a cost paid on every step, so the fast path
        # turns it off
        ...
        # DEBUG level logging to both a file and the console costs more per
        # control step than the simulation itself does, so the fast path keeps
        # only the messages a user needs to see
        ...
        if self.fast:
            ...
            # so set the level explicitly rather than relying on it
    ...
    def _fast_initialize(self):
        '''Caches what the fast simulation path needs, so that no per-step
        work is repeated.

        The default path builds a pyfmi result handler for every control step,
        which writes the variables selected by ``self.options['filter']`` to a
        result file and then reads them back.  The fast path reads the same
        variables straight out of the FMU with one bulk call per FMI type per
        communication point, which needs their value references up front.
        ...
        # Split the filtered variables by FMI type. Across all test cases
        # distributed with BOPTEST, the overwrite activation flags are the
        # only Booleans and everything else recorded is a Real. Anything else
        # would need its own bulk getter, so refuse rather than read it as a
        # Real: this path is opt-in, so failing at construction is better than
        # returning wrong measurements.
    ...
    def _fast_record(self, res):
        '''Appends the current FMU state to a results dictionary, in the form
        the pyfmi results object exposes to ``__get_results``.
    ...
    def _fast_simulation(self,start_time,end_time,input_object=None):
        '''Advances the FMU over one control step by calling ``do_step``
        directly, instead of calling ``fmu.simulate`` and letting pyfmi build
        a simulation algorithm and result handler for the step.

        The communication grid is identical to the default path: pyfmi's
        co-simulation algorithm steps ``self.options['ncp']`` times with
        ``h = (end_time-start_time)/ncp``, recording the start point and then
        each point reached, and this reproduces exactly that sequence of
        ``do_step`` calls and recorded points.  Results are therefore the
        same, and so are the KPIs computed from them.
        ...
        -------
        res: dict
            Maps each variable in ``self.options['filter']`` to a list of its
            values at the communication points, plus 'time'.  Returns a
            traceback string on failure, as ``__simulation`` does.
        ...
        try:
            ...
            if self.initialize_fmu:
                self.fmu.setup_experiment(start_time=start_time,
                                          ...
                                          stop_time=end_time)
                self.fmu.initialize()
            # Set control inputs. The input object holds a single time point,
            # so the trajectory pyfmi would interpolate is constant over the
            # step and setting it once is equivalent.
    ...
    def __simulation(self,start_time,end_time,input_object=None):
        ...
        # Take the low-overhead path if it was enabled. The sample rate is
        # resolved above, so both paths step the FMU on exactly the same grid.
        if self.fast:
