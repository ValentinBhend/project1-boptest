...
class KPI_Calculator(object):
    ...
    def _core_kpi_calculators(self, price_scenario):
        '''Map each core KPI name to the method that computes it.
...
#           ('atvl_tot', self.get_actuator_travel),   // While implemented here, can't be used until test cases updated
    ...
    def get_core_kpi_names(self):
        ...
        return list(self._core_kpi_calculators('Constant').keys())
    ...
    def get_core_kpis(self, price_scenario='Constant', names=None):
        ...
        names : list of str, optional
            Names of the core KPIs to compute. KPIs not named are skipped and
            left out of the result. This is safe to mix with full requests,
            because each KPI keeps its own integration index, so one that is
            requested later still integrates over the whole test period.
            Useful because the peak-demand KPIs are, by definition, maxima
            over the whole test period and so re-read the entire stored
            trajectory on every call, while the integrated KPIs do not.
            Default is None, which computes all of them.
